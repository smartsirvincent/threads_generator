...
// V2 輪詢窗口拉長:KIE V2 (gpt-image-2) 生成有時 >3 分鐘。
// 貼齊 Vercel function maxDuration(300s):留 buffer 給 submit/download/upload。
const V2_POLL_INTERVAL_MS = 3000;
const V2_POLL_MAX_ATTEMPTS = 85; // 85 * 3s = 255s

/**
 * V2 輪詢直到完成,回傳第一張結果 URL
 */
export async function pollImageV2(taskId) {
  for (let i = 0; i < V2_POLL_MAX_ATTEMPTS; i++) {
    const resp = await getJSON(`/jobs/recordInfo?taskId=${taskId}`);
    const d = resp.data;
    if (!d) {
      await sleep(V2_POLL_INTERVAL_MS);
      continue;
    }
    if (d.state === 'success') {
      try {
  ...
  throw new Error(`KIE V2 poll timeout after ${V2_POLL_MAX_ATTEMPTS * V2_POLL_INTERVAL_MS / 1000}s`);
}
