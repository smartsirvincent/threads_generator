...
import { normalizeInput, productsBriefForPrompt, avoidPromptHint, getEnabledProductsForImageGen, weightedPool } from './normalize.js';
...
function themePool(theme, input) {
  ...
  if (theme.type === 'product_with_image') {
    ...
    if (enabled.length > 0) return weightedPool(enabled);
  }
  return all.length > 0 ? weightedPool(all) : [{ product: { name: '療程', features: '' }, originalIndex: 0 }];
}
...
function buildBatchPrompt({ theme, input, batchIndex, batchCount, totalSoFar, previousTitles }) {
  ...
  // productsList 顯示時用 originalIndex,讓 LLM 學會使用真實療程 index;附上比重
  const productsList = pool
    ...
    .join('\n\n');
  const hasWeights = pool.length > 1 && pool.some((e) => (e.product?.weight || 1) !== (pool[0].product?.weight || 1));
  const weightInstr = hasWeights
    ? `\n**比重分配**: 各療程後方的「比重」代表發文比例,比重越高的療程請安排越多篇(比重 3 約為比重 1 的三倍),請盡量依比重分配這批貼文。`
    : '';
...
**療程清單(共 ${products.length} 個)**:
${productsList}${weightInstr}
