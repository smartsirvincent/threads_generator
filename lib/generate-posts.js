...
function buildBatchPrompt({ theme, input, batchIndex, batchCount, totalSoFar, previousTitles }) {
  ...
  const productsList = productsBriefForPrompt(products);
...
**重要**: product_index 必須是 0 到 ${products.length - 1} 之間的整數,代表這篇文案是針對哪個 SKU。${products.length > 1 ? '這批 ' + batchCount + ' 篇要盡量輪替不同 SKU,不要全寫同一個。' : ''}`,
...
**重要**: product_index 必須是 0 到 ${products.length - 1} 之間的整數。${products.length > 1 ? '這批要盡量輪替不同 SKU。' : ''}`,
...
export async function generateOneBatch({ theme, input: rawInput, count, previousTitles = [] }) {
  const input = normalizeInput(rawInput);
  const safeCount = Math.min(Math.max(1, count || BATCH_SIZE), BATCH_SIZE);
  const { system, user } = buildBatchPrompt({
    theme,
    input,
...
export function generateThemePostsDryRun({ theme, input: rawInput }) {
  ...
  const samples = {
    product_with_image: (i) => {
      // 輪替 SKU
      const pi = (i - 1) % input.products.length;
      const p = input.products[pi];
      return {
        product_index: pi,
