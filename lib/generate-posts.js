...
**購買連結**: ${input.purchase_url || '(未提供)'}
**產品圖 URL(隨機挑一張使用)**: ${(input.product_images || []).join(' / ') || '(未提供)'}
...
**主題提示**: ${type.promptHint}`;
    ...
    product_with_image: `[
  {
    ...
    "Prompt核心關鍵字": "AI 圖片 prompt 關鍵字英文"
  }
]`,

    product_with_url: `[
  { "主題分類": "迷思破解|功能介紹|使用情境|限時優惠...",
    "文案內容": "完整 Threads 貼文(結尾含購買連結 ${input.purchase_url || '網址'} + hashtag)" }
]`,
 ...
 * 生成單一主題的所有貼文,回傳貼文陣列
 */
export async function generateThemePosts({ theme, input, onProgress }) {
 ...
 */
export function generateThemePostsDryRun({ theme, input }) {
  const count = theme.monthly_count || 30;
  const posts = [];
  const samples = {
    product_with_image: (i) => ({
      ...
      核心痛點: '時間不夠', 產品亮點: input.product, 情緒語法: '安定感',
      CTA: '留言告訴我你的選擇', 場景設定: '加班深夜', 字數: 200,
      文案內容: `[Dry-run] ${theme.name} 第 ${i} 篇\n\n${input.product_features.slice(0, 50)}...`,
      Prompt核心關鍵字: 'cozy modern lifestyle photography',
    }),
    product_with_url: (i) => ({
      主題分類: '功能介紹',
      文案內容: `[Dry-run] ${theme.name} 第 ${i} 篇\n${input.product_features.slice(0, 80)}\n${input.purchase_url || ''}`,
    }),
    opinion_short: (i) => ({
      主題分類: '問題意識',
