...
export async function listRawResources({ prefix, maxResults = 100 }) {
  // Cloudinary search API:更即時,新上傳的資源 < 1 分鐘內可查到
  const searchUrl = `https://api.cloudinary.com/v1_1/${cloudName()}/resources/search`;
  const body = {
    expression: `resource_type:raw AND folder:${prefix.replace(/\/$/, '')}`,
  ...
  // search 失敗 fallback 到 list API
  const listUrl = `https://api.cloudinary.com/v1_1/${cloudName()}/resources/raw`
    + `?prefix=${encodeURIComponent(prefix)}`
    + `&max_results=${maxResults}`
    + `&type=upload`;
  const listRes = await fetch(listUrl, { headers: { 'Authorization': adminAuth() } });
  if (!listRes.ok) {
    const text = await listRes.text();
    throw new Error(`Cloudinary list/search HTTP ${listRes.status}: ${text}`);
  }
  const listData = await listRes.json();
  return listData.resources || [];
}
 ...
 * 回傳直接可下載的 URL
 */
export async function uploadRawToCloudinary(buffer, { folder, publicId, filename = 'output.xlsx' } = {}) {
  const mode = uploadMode();
