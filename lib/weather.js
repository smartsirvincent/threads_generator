// 曼谷天氣抓取 + 天氣提醒貼文產生(給 /api/post/weather 與 cron 每日定時共用)
...
export async function fetchBangkokWeather() {
...
export function weatherLine(wx) {
  return [
    `天氣:${wx.desc}`,
...
用詞硬規則:提到地陪/翻譯/隨行/客服一律中性、不寫性別(不出現男/女/帥哥/美女/他/她);診所下午 1 點後才營業,不要寫上午/一早去做醫美或療程(要提時間寫下午或傍晚)。
格式:Threads 是純文字,禁止使用任何 markdown 符號(不要 ** 粗體、__、# 標題、> 引用、\`\`\`);要強調用文字或 emoji。
輸出 JSON(嚴格):{"text":"完整貼文(含換行與 hashtag)"}`;

// 抓天氣 + 產文,回 { weather, weatherLine, text }
export async function makeWeatherPost({ brand, brand_persona, audience, clinic } = {}) {
  const wx = await fetchBangkokWeather();
  const wl = weatherLine(wx);
  const clinicText = clinicContextText(clinic);
  const user = `**曼谷今日天氣**: ${wl}
...
請依上面的天氣寫一則「天氣提醒 + 保養/術後照護」貼文,直接回 JSON。`;
  ...
  return { weather: wx, weatherLine: wl, text };
}
