...
import { normalizeInput, productsBriefForPrompt, avoidPromptHint, enabledImageStyles, logoPromptHint } from './normalize.js';
  ...
  "themes": [
    {
      ...
      "image_style": "scene" | "character" | "product",
...
export async function recommendImageThemes(rawInput) {
  ...
  const styles = enabledImageStyles(input);
  const styleNames = styles.map((s) => ({ scene: '情境圖', character: '人物搭配', product: '產品為主' })[s]).join(' / ');
...
**用戶啟用的圖片風格**: ${styleNames}
→ 主題 image_style 只能用以下:${styles.map((s) => `"${s}"`).join(', ')}
→ 推薦時請在這幾種風格之間平均分配主題數,不要全部擠在一種
...
請推薦 5–8 個圖片主題,涵蓋多種視覺風格。`;
  ...
  // 強制 type 鎖在 product_with_image,image_style 不在啟用清單則改成第一個
  result.themes = result.themes.map((t) => ({
    ...t,
    type: 'product_with_image',
    image_style: styles.includes(t.image_style) ? t.image_style : styles[0],
  }));
...
export function recommendImageThemesDryRun(rawInput) {
  const input = normalizeInput(rawInput);
  const styles = enabledImageStyles(input);
  const pickStyle = (i) => styles[i % styles.length];
  const dry = [
