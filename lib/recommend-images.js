...
**image_style 二選一**:
- "scene"      情境圖,生活場景+環境敘事(診間/曼谷生活/療癒氛圍)
- "character"  人物模特/膚況主體(透亮膚況、自然表情)
...
**image_style 二選一,必填**:
- "scene":情境/環境/敘事感(診間角落、曼谷生活、療癒氛圍)
- "character":人物/膚況主體(模特兒、透亮膚況、自然表情)`;
...
export async function recommendImageThemes(rawInput) {
  ...
  const strategyInstr = strategy === 'per_sku'
    ? `**策略: 一療程一專屬主題**
每個療程各 1 個獨立主題,主題名要明確點出療程名稱。
- 推薦剛好 ${enabled.length} 個主題,一個療程一個 (不要多)
- 每個主題在輸出加 "locked_product_index" 欄位指定該療程的 # (例如 ${enabled[0].originalIndex})
- image_style 必須在該療程的「可接受風格」清單內
- 每個 visual_style 控制在 30 字內,精簡即可`
    : `**策略: 共用主題輪替 SKU**
