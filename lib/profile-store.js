...
// 讀取固定槽的雲端品牌設定 (跨裝置一致)。抓不到回 null。
export async function loadCanonicalProfile(name) {
  if (!name) return null;
  try {
    const r = await fetch(`/api/profiles/get?name=${encodeURIComponent(name)}`, { cache: 'no-store' });
    if (!r.ok) return null;
