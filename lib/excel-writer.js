 ...
 */
function writeThemeSheet(wb, theme, posts, startDate) {
  ...
  posts.forEach((post, i) => {
    ...
    for (const col of type.columns) {
      ...
      else if (col === '發文時間') rowData[col] = formatDateTime(dates[i]);
      else if (col === '產品圖') rowData[col] = pickRandom(post._productImages) || '';
      else if (col === 'AI圖') rowData[col] = ''; // 留空,後續可接圖片生成
      else if (col === '網址') rowData[col] = post.網址 || post._purchaseUrl || '';
...
function writeProductSheet(wb, input) {
  ...
  const images = (input.product_images || []).join('\n');
  const row = ws.addRow({
    '產品': input.product,
    '產品特色': input.product_features,
    '產品圖': images,
    '購買連結': input.purchase_url || '',
  });
  row.alignment = { vertical: 'top', wrapText: true };
 ...
 */
function buildWorkbook({ input, themes, postsByTheme, startDate }) {
  ...
  for (const theme of themes) {
    ...
    posts.forEach((p) => {
      p._productImages = input.product_images;
      p._purchaseUrl = input.purchase_url;
    });
    writeThemeSheet(wb, theme, posts, startDate);
  }
