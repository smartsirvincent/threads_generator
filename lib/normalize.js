...
export function normalizeImageStyles(raw) {
  const r = raw || {};
  // 醫美只保留 情境(scene) / 人物(character);產品與電商促銷已移除
  let out = {
    scene: r.scene !== false,
  ...
  // 至少要有一個
  if (!out.scene && !out.character) {
    out = { scene: true, character: true, product: false, ecommerce: false };
  }
  return out;
}
...
/**
 * 依 weight 展開池:weight 越高,在陣列中重複越多次,round-robin 自然取得較高比重
 * 回傳 [{ product, originalIndex }, ...] (已依比重重複)
 */
export function weightedPool(entries) {
  const out = [];
  for (const e of entries) {
    const w = normalizeWeight(e.product?.weight);
