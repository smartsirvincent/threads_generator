    ...
    <main className="space-y-6">
      <div className="card border-indigo-200 bg-indigo-50/40">
        ...
        <div className="mt-3 flex flex-wrap items-end gap-3">
          <button type="button" onClick={() => quick(7)} className="rounded-md border border-stone-300 bg-white px-3 py-1.5 text-sm text-stone-600 hover:bg-stone-50">近 7 天</button>
          <button type="button" onClick={() => quick(30)} className="rounded-md border border-stone-300 bg-white px-3 py-1.5 text-sm text-stone-600 hover:bg-stone-50">近 30 天</button>
          ...
          <button type="button" onClick={() => load()} disabled={busy} className="rounded-md bg-indigo-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50">{busy ? '讀取中…' : '查詢'}</button>
        </div>
        {data && !hasInsights && <p className="mt-2 text-xs text-amber-600">未連 Threads insights → 僅顯示發文數。設定 THREADS_ACCESS_TOKEN 並開 threads_manage_insights 權限後即可看瀏覽/互動。（收藏數 Threads API 未提供）</p>}
      </div>
      ...
      {t && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[['發文數', t.count], ['總瀏覽', hasInsights ? t.views.toLocaleString() : '—'], ['總互動', hasInsights ? t.engagement.toLocaleString() : '—'], ['互動率', hasInsights ? t.rate + '%' : '—']].map(([k, v]) => (
            <div key={k} className="card text-center"><div className="text-xs text-stone-500">{k}</div><div className="mt-1 text-2xl font-semibold text-stone-900">{v}</div></div>
          ))}
        </div>
      )}
      ...
      {/* 依主題 */}
      <Section title="依主題" action={<span className="text-[11px] text-stone-400">排序:{hasInsights ? '瀏覽數' : '發文數'}</span>}>
        <MetricTable rows={(data?.byTopic || []).map((r) => ({ label: r.topicName, ...r }))} nameHead="主題" cols={METRIC_COLS} hasInsights={hasInsights}
          extra={(r) => <a href="/post" className="text-sky-600 underline">用此主題產文</a>} />
      </Section>

      {/* Top 10 */}
      <Section title="期間最佳 10 篇貼文">
        {(!data?.topPosts || data.topPosts.length === 0)
          ? <p className="text-sm text-stone-400">{hasInsights ? '此期間尚無有數據的貼文。' : '需連 Threads insights 才能排名。'}</p>
          : (
            <div className="space-y-1.5">
              {data.topPosts.map((p, i) => (
                <div key={i} className="flex items-start gap-2 rounded-lg border border-stone-200 p-2 text-xs">
                  <span className="mt-0.5 w-5 shrink-0 text-center font-semibold text-stone-400">{i + 1}</span>
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-stone-800">{p.textPreview || '(無預覽)'}</div>
                    <div className="text-stone-500">{p.topicName} · {({ text: '純文字', long: '長文', image: '圖片' })[p.type] || p.type} · {new Date(p.ts).toLocaleDateString('zh-TW')}{p.permalink && <> · <a href={p.permalink} target="_blank" rel="noreferrer" className="text-sky-600 underline">看貼文↗</a></>}</div>
                  </div>
                  <div className="shrink-0 text-right text-stone-600">👁{p.views.toLocaleString()}<br />❤{p.likes} 🔁{p.reposts} 💬{p.replies}</div>
                </div>
              ))}
            </div>
          )}
...
function Section({ title, action, children }) {
  return (
    <div className="card space-y-2">
      <div className="flex items-center justify-between"><h2 className="text-sm font-semibold text-stone-800">{title}</h2>{action}</div>
...
function MetricTable({ rows, nameHead, cols, hasInsights, extra }) {
  if (!rows || rows.length === 0) return <p className="text-sm text-stone-400">此期間尚無資料。</p>;
  return (
