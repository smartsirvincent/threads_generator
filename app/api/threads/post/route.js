...
import { getThreadsCreds, publishThreads, appendPostLog } from '@/lib/threads.js';
...
export async function POST(req) {
  try {
    ...
    const { text, topicId = '', topicName = '', type = '', imageUrl = '' } = await req.json();
    const { mediaId, permalink } = await publishThreads(text, imageUrl);
